...
module Battler =
    ...
    let fight
            ...
            (getTeam: GetTeam) =
        async {
            let logStatus = Logger.logStatus logger
            do! logStatus FightLog.CheckingForExistingBattle
            let! matchInfo  = OutstandingMatch.getOutstandingMatch username

            match matchInfo.IsSome && matchInfo.Value.team_hash <> "" with
            | true -> do! logStatus FightLog.BattleAlreadyStarted
            | _ ->
                do! logStatus FightLog.StartNewMatch
